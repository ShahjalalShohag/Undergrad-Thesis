...
// delete p% nodes randomly and store the modified graph in gnxt
void node_deletion(int n, int m, graph &g, double p, int &nnxt, int &mnxt, graph &gnxt) {
  vector<int> nodes;
  for (int i = 0; i < n; i++) {
    nodes.push_back(i);
  }
...
//   for (int u = 0; u < n; u++) {
//     for (auto v: g[u]) {
//       if (newid[u] != -1 and newid[v] != -1) {
//         gnxt[newid[u]].push_back(newid[v]);
//         gnxt[newid[v]].push_back(newid[u]);
//         ++mnxt;
//       }
//     }
//   }
...
double core_resilience(double r, double p, int n, int m, graph &g, vector<int> &core) {
  ...
  node_deletion(n, m, g, p, nnxt, mnxt, gnxt);
